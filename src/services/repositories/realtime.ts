...
export async function fetchInitialData<K extends SyncedTable>(
  ...
  const builder =
    actualTable === "org_client_stats_view"
      ? supabase.from("org_client_stats_view")
      : supabase.from(actualTable as keyof Database["public"]["Tables"]);

  let query = builder.select(SELECTS[table]).eq("org_id", orgId);
