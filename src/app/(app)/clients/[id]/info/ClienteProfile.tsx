...
import { EditClientDialog } from "./EditClient";
...
export default function ClientProfile({ initialData }: { initialData: Client }) {
  const { data: client, mutate } = useSWR<Client>(`/api/client/${initialData.id}`, fetcher, {
  ...
  async function handleSave(updated: Partial<Client>) {
    try {
      ...
      mutate(); // volta pro cache original
    }
  }
    ...
    <div className="max-w-5xl mx-auto p-10 space-y-8 bg-white/90 backdrop-blur rounded-3xl shadow-lg">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <Card className="p-5">
          ...
          <h3 className="font-semibold mt-1">{client.payment_date || "—"}</h3>
        </Card>
      </div>
